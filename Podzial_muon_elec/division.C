...
eg_cluster_phi, type: vector<float>
   */
   ...
   TFile *output = new TFile("data_separated.root", "recreate");
   
   TTree *tree_electron = new TTree("tree_electron", "new_tree2");
   tree_electron->Branch("track_TRTHits", &track_TRTHits);
   tree_electron->Branch("track_PixelHits", &track_PixelHits);
   tree_electron->Branch("track_SCTHits", &track_SCTHits);
   tree_electron->Branch("track_PixeldEdX", &track_PixeldEdX);

   TTree *tree_muon = new TTree("tree_muon", "new_tree2");
   tree_muon->Branch("track_TRTHits", &track_TRTHits);
   tree_muon->Branch("track_PixelHits", &track_PixelHits);
   tree_muon->Branch("track_SCTHits", &track_SCTHits);
   tree_muon->Branch("track_PixeldEdX", &track_PixeldEdX);
   ...
   //single but hits:
   TH1F *single_elec_trt = new TH1F("single_elec_trt", "single_elec_trt", 50, 0, 60);
   TH1F *single_elec_sct = new TH1F("single_elec_sct", "single_elec_sct", 50, 0, 16);
   TH1F *single_elec_pixel = new TH1F("single_elec_pixel", "single_elec_pixel", 50, 0, 10);

   TH1F *single_muon_trt = new TH1F("single_muon_trt", "single_muon_trt", 50, 0, 60);
   TH1F *single_muon_sct = new TH1F("single_muon_sct", "single_muon_sct", 50, 0, 16);
   TH1F *single_muon_pixel = new TH1F("single_muon_pixel", "single_muon_pixel", 50, 0, 10);
            ...
            if (abs((*track_eta)[0]) <= eta_max && abs((*track_eta)[1]) <= eta_max)
            {
                ...
                {  
                    ...
                    is_muon = 1;
                        ...
                        if(eg_cluster_n >= 1)
                        {
                                int cluster_best=-1, track_best=-1; //Lowest Delta R
                        ...
                        if (vsum.Pt() <= 0.2)
                        {
                            if(is_muon == 1)
                            {
                                if(vsum.M() >= 2.9 && vsum.M() <= 3.2)
                                {
                                    ...
                                    single_muon_trt->Fill((*track_TRTHits)[0]);
                                    single_muon_trt->Fill((*track_TRTHits)[1]);
                                    single_muon_sct->Fill((*track_SCTHits)[0]);
                                    single_muon_sct->Fill((*track_SCTHits)[1]);
                                    single_muon_pixel->Fill((*track_PixelHits)[0]);
                                    single_muon_pixel->Fill((*track_PixelHits)[1]);
                                }
                            }
                            ...
                            else if (is_elec ==1 )
                            {
                                ...
                                if (electron.E() != 0)
                                {
                                    ...
                                    single_elec_trt->Fill((*track_TRTHits)[elec_track]);
                                    single_elec_sct->Fill((*track_SCTHits)[elec_track]);
                                    single_elec_pixel->Fill((*track_PixelHits)[elec_track]);
                                }
                            }
                        }
...
cout<<endl<<"Single Muons: "<<num_muon*2<<endl<<"Single Electrons: "<<num_elec;
...
TCanvas *c4 = new TCanvas("c4", "Hits_muon_elec", 1200, 800);
c4->Divide(2,3);
// -electrons
c4->cd(1);
single_elec_trt->Draw();

c4->cd(2);
single_elec_sct->Draw();

c4->cd(3);
single_elec_pixel->Draw();

// muons
c4->cd(4);
single_muon_trt->Draw();

c4->cd(5);
single_muon_sct->Draw();

c4->cd(6);
single_muon_pixel->Draw();
c4->SaveAs("Hits_muon_elec.png");
}