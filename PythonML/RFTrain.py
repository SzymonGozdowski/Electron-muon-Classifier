...
# ------ Correct false positives to feed neural network ------
df_test = df.loc[idx_test].copy()
df_test["y_pred"] = y_pred
df_test["proba"] = probs

df["IsMuonRF"] = df["IsMuon"].copy()
mask_e = (df_test["IsMuon"] == 0) & (df_test["proba"] >= 0.85)
mask_mu = (df_test["IsMuon"] == 1) & (df_test["proba"] <= 0.15)

df.loc[df_test.index[mask_e], "IsMuonRF"] = 1
df.loc[df_test.index[mask_mu], "IsMuonRF"] = 0
print("Corrected false positives")
"""
if mask.empty:
