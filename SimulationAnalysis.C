...
void SimulationAnalysis()
{
   ...
   if(choice==0)
   {
    files.at(0)="data23_2trk_moreTCvars.root";
    name="TrueData";
   }
   if(choice==1)
   {
    files.at(0)="mc_jpsi_ee.root";
    name="MCElectron";
   }
   if(choice==2)
   {
    files.at(0)="mc_jpsi_mumu.root";
    name="MCMuon";
   }
   ...
   TFile *file = new TFile(Form("MLData%s.root",name.c_str()), "RECREATE");
   ...
   for(int File=0; File<NumOfFiles;File++)
   {
    ...
    while(tree_reader.Next()){
        eventID++;
        if(eventID%100000==0) cout<<"Processing "<<eventID<<" event..."<<endl;
